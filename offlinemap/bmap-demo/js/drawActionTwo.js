...
var drawingManagerEntity = function () {
    ...
    //1:画点2:画线 3:画矩形4:画圆5:画多边形   6:添加多边形功能
    var _openDrawtwo = function (DrawingType) {
        _closeDraw();
        if (DrawingType == 1) {
...
//新建一个绘制管理工具对象
var publicDrawingManagertow = new drawingManagerEntity();
...
var overlaycomplete = function (e,num) {
    ...
    if (whitchOver == 3) {
        ...
        var circlePositon = publicDrawingManagertow.getLastOverlay(3).getCenter(); //中心点
...
publicDrawingManagertow.getDrawManager().addEventListener('overlaycomplete', overlaycomplete);
//清空绘制存放的数组
function clearAll() {
    //清空覆盖物数组与地图上的覆盖物
    publicDrawingManagertow.clearStoreAndMap(map);
    //清除绘制的覆盖物列表
...
function closeDraw() {
    publicDrawingManagertow.closeDraw();
}
